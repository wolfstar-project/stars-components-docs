...
<Tabs groupId="package-manager" persist items={['pnpm', 'npm', 'yarn', 'bun']}>
	<Tab value="pnpm">```bash pnpm add @wolfstar/plugin-i18next ```</Tab>
	<Tab value="npm">```bash npm install @wolfstar/plugin-i18next ```</Tab>
	<Tab value="yarn">```bash yarn add @wolfstar/plugin-i18next ```</Tab>
	<Tab value="bun">```bash bun add @wolfstar/plugin-i18next ```</Tab>
</Tabs>
