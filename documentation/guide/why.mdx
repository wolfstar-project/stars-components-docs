...
[`@wolfstar/http-framework`](/documentation/packages/http-framework) owns exactly that surface: the HTTP server, Discord request
...
Adopting [`@wolfstar/safe-fetch`](/documentation/packages/safe-fetch) or [`@wolfstar/logger`](/documentation/packages/logger) in an existing
service costs one dependency and no framework buy-in. See [Bot Architecture](/documentation/guide/architecture) for how a bot is put together.
...
`@wolfstar/http-framework` itself. See the [Plugins](/documentation/guide/plugins) guide.
...
implementation files never become accidental public contracts. The [API reference](/documentation/api) is generated from those entry
points on every documentation build, so it cannot drift from the code.

The same idea runs through the packages themselves: typed environment keys in
[`@wolfstar/env-utilities`](/documentation/packages/env-utilities), typed translation keys in
[`@wolfstar/plugin-i18next`](/documentation/packages/plugin-i18next), `Result` values instead of thrown errors in
[`@wolfstar/safe-fetch`](/documentation/packages/safe-fetch).
...
[`@wolfstar/http-framework-test-utils`](/documentation/packages/http-framework-test-utils) dispatches interaction fixtures without
starting a network server. See [Testing Interactions](/documentation/guide/testing).

## Next Steps

- [Features](/documentation/guide/features) — what ships in the box
- [Getting Started](/documentation/guide) — create a bot in one command
