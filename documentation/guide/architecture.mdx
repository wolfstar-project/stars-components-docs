...
<Warning>`@wolfstar/http-framework-i18n` is deprecated in favour of `@wolfstar/plugin-i18next`. See the [migration guide](/guide/migration).</Warning>
...
[API reference](/api/index) is generated from those entry points during every documentation build, so private implementation
