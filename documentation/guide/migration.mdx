...
- [ ] CLI-only consumers (projects that depend on `@wolfstar/cli` without `@wolfstar/http-framework`) added
      `@wolfstar/http-framework` to their `dependencies` — the CLI no longer depends on the framework transitively.
      Projects scaffolded by `@wolfstar/create-http-framework`, or that already depend on `@wolfstar/http-framework`,
      are unaffected.
...
[`@wolfstar/http-framework-i18n`](/documentation/packages/http-framework-i18n) is **deprecated**. Its successor is
[`@wolfstar/plugin-i18next`](/documentation/packages/plugin-i18next), an official plugin for
[`@wolfstar/http-framework`](/documentation/packages/http-framework) that lives in the
