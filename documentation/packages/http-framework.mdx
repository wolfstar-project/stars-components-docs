...
description: 'HTTP-only Discord bot framework built on node:http, @discordjs/rest, and @sapphire/pieces, with decorator-based command registration and HMR.'
---
...
`@wolfstar/http-framework` owns the typed project configuration consumed by the [`stars` CLI](/documentation/packages/cli) — the
