...
calls into it), the typed `stars.config.*` schema and loader live in [`@wolfstar/http-framework`](/documentation/packages/http-framework)
...
```sh
stars dev [--no-tui] [--theme <name>] [--config <file>] [--cwd <dir>]
...
| `T`            | pick a colour theme (see [Themes](#themes))                  |
| `l`            | browse logs                                                  |
...
| `Ctrl+C`       | start a graceful shutdown; press `Ctrl+C` again to force quit (`SIGKILL`s the bot and exits) |
...
Press `T` in the interactive UI to open the theme picker. Arrows (or `j`/`k`) preview a theme live across the whole
panel, `Enter` saves the highlighted theme, and `Esc` restores the theme that was in effect when the picker opened.

Seven themes ship built-in:

| Setting            | Description                                                                        |
| ------------------ | ---------------------------------------------------------------------------------- |
| `auto`             | Follows the terminal background through `COLORFGBG`; falls back to `dark`          |
| `dark`             | For dark terminals                                                                 |
| `light`            | For light terminals                                                                |
| `dark-daltonized`  | Dark, colour-blind friendly (blue/orange instead of green/red, Okabe-Ito palette)  |
| `light-daltonized` | Light, colour-blind friendly (blue/orange instead of green/red, Okabe-Ito palette) |
| `dark-ansi`        | Dark, uses only the 16 ANSI colours so the terminal palette decides                |
| `light-ansi`       | Light, uses only the 16 ANSI colours so the terminal palette decides               |

The theme in effect for a run resolves in this order: `--theme <name>` › `STARS_THEME` › the saved preference › `auto`.
An invalid `--theme` value exits with `INVALID_THEME`. An invalid `STARS_THEME` is ignored so an environment variable
does not stop the dev server.

```sh
stars dev --theme light-daltonized     # one-off override
STARS_THEME=dark-ansi stars dev        # override through the environment
```

The saved preference lives in `preferences.json` under the first of `$STARS_CONFIG_DIR`, `$XDG_CONFIG_HOME/stars`,
`%APPDATA%\stars` (Windows), or `~/.config/stars` that is set.
...
a dumb terminal. The plain renderer is unthemed; themes only apply to the interactive UI. Both modes honour
`NO_COLOR`/`FORCE_COLOR`, and `NO_COLOR` still disables colour even when a theme is set. `STARS_REDUCED_MOTION=1`
freezes the logo/spinner but keeps the elapsed clock. Both stop the bot cleanly on `SIGINT`/`SIGTERM`; a second
`SIGINT`/`SIGTERM`/`SIGHUP` skips the graceful wait and `SIGKILL`s the bot. `SIGUSR2` restarts the bot (not on Windows).
