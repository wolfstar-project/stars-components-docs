...
<Note>
This class extends `Client` from `@wolfstar/http-framework`. Its inherited surface (properties like `options`, `logger`, `server`, `plugins`, the `registry` accessor, and `EventEmitter`-style methods like `on`/`once`/`emit`/`use`) isn't reproduced here — see the [http-framework API reference](/api/http-framework) for that class's full documentation. Only the members this class adds or overrides are listed below.
</Note>
